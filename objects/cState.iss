...
objectdef cState inherits cBase
{
	...
	method Pulse()
	{
		...
		if !${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked} && !${UIElement[chkPartyMode@Overview@Pages@Twisted].Checked}
		{		
		...
		elseif ${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
		{
	...
	member DEADState()
	{
		...
		if ${Me.Ghost}
		{
			if ${Math.Calc[${LavishScript.RunningTime} - ${This.JustRes}]} >= ${LagTime}
			{
				...
				if ${POI.Type.Equal[SPIRITHEALER]} && !${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
				{
					if ${POI.InUseRange}
					{
				...
				{
					if ${POI.SetNearestPOI[SPIRITHEALER]} && !${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
					{
						return CB_DEADMOVE
					}
	...
	method DEADPulse()
	{
		switch ${This.CurrentSubState}
		{
			...
			case CB_RELEASE
			if ${UIElement[chkDeathSoundOn@Config@Pages@Twisted].Checked}
			{
			...
			WoWScript RepopMe()
			if ${LavishScript.RunningTime} <= ${Math.Calc[${POI.LastRetrieveCorpseTime} + ${POI.CorpseCampTimeout}]} && !${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
			{
	...
	{	
		...
		if ${Human.NeedAction} && ${Human.ActionDelay} < ${LavishScript.RunningTime} && !${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
		{
			return CB_ACT_HUMAN
		}
	...
	member LOOTState()
	{
		...
		if ${LavishScript.RunningTime} >= ${LOOTState_Skip_Scans} || ${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
		{
			
			This.LOOTState_Skip_Scans:Set[${Math.Calc[${LavishScript.RunningTime}+(${This.LOOTState_Skip_Scans_Timeout} * ${Bot.GlobalCooldown})]}]

			if (${Inventory.FreeSlots} > 0) && ${UIElement[chkLoot@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				list:Clear
			...
			if !${Found} && ${Toon.HasSkill[SKILL_HERBALISM]} && (${Inventory.FreeSlots} > 0) && ${UIElement[chkGather@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				list:Clear
			...
			if !${Found} && ${Toon.HasSkill[SKILL_MINING]} && ${Item[-inventory,"Mining Pick"](exists)} && (${Inventory.FreeSlots} > 0) && ${UIElement[chkGather@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				list:Clear
			...
			/* quest objects */
			if !${Found} && (${Inventory.FreeSlots} > 0) && ${UIElement[chkHarvestQuests@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				list:Clear
	...
	member NPCState()
	{
		...
		if ${POI.NeedLogout} || (${UIElement[chkLogOutOnLevel@Logout@Pages@Twisted].Checked} && ${UIElement[sldLogOutLevel@Logout@Pages@Twisted].Value(exists)} && ${Me.Level} >= ${UIElement[sldLogOutLevel@Logout@Pages@Twisted].Value}) || (${UIElement[chkLogoutOnTimer@Logout@Pages@Twisted].Checked} && ${UIElement[sldLogOutIn@Logout@Pages@Twisted].Value(exists)} && ${Math.Calc[${${Script.RunningTime}.RunningTime}/60000]} >= ${UIElement[sldLogOutIn@Logout@Pages@Twisted].Value})
		{
	...
	method NPCPulse()
	{
		switch ${This.CurrentSubState}
		{
			...
			case CB_LOGOUT
			if ${UIElement[chkErrorSoundOn@Config@Pages@Twisted].Checked}
			{
				This:PlaySound["Stop"]
			}				
	 		...
	 		if ${Object[${Inventory.GetSlot[Mule,"-notsoulbound"]}](exists)} && ${Me.Coinage} >= 30
			{
				...
				{
					WoWScript SendMail(\"${UIElement[tenMuleChar@Config@InvPages@Inventory@Pages@Twisted].Text}\"\, \"${Object[${Inventory.GetSlot[Mule,"-notsoulbound"]}].Name}\"\, \"\")
				}
				else
				{
	...
	member QUESTState()
	{
		if ${UIElement[chkPlayQuest@Quest@HumanPages@Human@Pages@Twisted].Checked}
		{
	...
	{		
		...
		{
			if ${Toon.HasSkill[Herbalism]} && ${UIElement[chkRoamHerb@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				canHerb:Set[TRUE]
			}
			
			if ${Item[-inventory,"Mining Pick"](exists)} && ${Toon.HasSkill[Mining]} && ${UIElement[chkRoamMine@Config@InvPages@Inventory@Pages@Twisted].Checked}
			{
				canMine:Set[TRUE]
			}
			if ${canHerb} && ${canMine}
