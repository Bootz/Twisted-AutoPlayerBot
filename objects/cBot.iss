objectdef cBot inherits cBase
{
	...
	{
		if ${UIElement[cmbBotGlobalCooldown@Overview@Pages@Twisted].Selection} != 0
		{
			This.GlobalCooldownBase:Set[${Math.Calc[250 * ${UIElement[cmbBotGlobalCooldown@Overview@Pages@Twisted].Selection}]}]
			This.CPUCooldownBase:Set[${Math.Calc[5 * ${UIElement[cmbBotGlobalCooldown@Overview@Pages@Twisted].Selection}]}]			
			This.LagInterval:Set[${Math.Calc[500 * ${UIElement[cmbBotGlobalCooldown@Overview@Pages@Twisted].Selection}]}]			
		}
	}
	...
	function Startup()
	{
		...
		This:Output["Twisted ${This.Version} is Loading...."]
		...
		This:Output["Twisted ${This.Version} is Online. All ur bot r belong 2 us."]
	}
	...
	{		
		...
		do
		{
			...
			{
				if ${UIElement[chkAutoReconnect@Logout@Pages@Twisted].Checked}
				{
					...
					do
					{
						if !${Me(exists)}
						{
							...
							WoWScript DefaultServerLogin("${UIElement[AccountName@Logout@Pages@Twisted].Text}", "${UIElement[Password@Logout@Pages@Twisted].Text}")
					...
					wait 100
					Press "${UIElement[EnterKey@Logout@Pages@Twisted].Text}"
					echo Entering world...
					do
					{
						wait 10
