objectdef cTargeting inherits cBase
{
	...
	{	
			...
			{	
				...
				{		
					...
					else
					{
						...
						if !${UIElement[chkPreferElites@Config@Pages@Twisted].Checked}
						{
		...
		if ${UIElement[chkAttackPvP@Config@Pages@Twisted].Checked}
		{
			Targeting.possiblePvPTargets:Search[-players,-pvp,-alive,-notflying,-attackable,-range 0-200]
		}	
		elseif ${Me.InCombat} && ${UIElement[chkDefendPvP@Config@Pages@Twisted].Checked}
		{
		...
		if ${Targeting.possiblePvPTargets.Count}
		{
			do
			{
				...
				if ${PossibleTarget(exists)} && ${PossibleTarget.Name.NotEqual[NULL]}
				{
					...
					if ${UIElement[chkDefendPvP@Config@Pages@Twisted].Checked} && (${PossibleTarget.Target.GUID.Equal[${Me.GUID}]} || ${PossibleTarget.Target.GUID.Equal[${Me.Pet.GUID}]})
					{
						Targeting.TargetCollection:Insert[${PossibleTarget.GUID}]			
					}
					elseif ${UIElement[chkAttackPvP@Config@Pages@Twisted].Checked}
					{
						Targeting.TargetCollection:Insert[${PossibleTarget.GUID}]			
					}
	...
	{
		variable int i = 1
		UIElement[lstTargets@Overview@Pages@Twisted]:ClearItems
		if ${Targeting.TargetCollection.Get[${i}](exists)}
		{
			do
			{
				UIElement[lstTargets@Overview@Pages@Twisted]:AddItem[${Object[${Targeting.TargetCollection.Get[${i}]}].Name}]
	...
	method MinMaxLvLs()
	{
		...
		if ${UIElement[chkIgnoreMax@Config@Pages@Twisted].Checked}
		{
			Targeting.MinLvL:Set[80]			
			Targeting.MaxLvL:Set[80]
		}
		if ${UIElement[chkIgnoreMin@Config@Pages@Twisted].Checked}
		{
			Targeting.MinLvL:Set[1]
		}		
		UIElement[txtMinLvL@Config@Pages@Twisted]:SetText[Min: ${Targeting.MinLvL}]
		UIElement[txtMaxLvL@Config@Pages@Twisted]:SetText[Max: ${Targeting.MaxLvL}]
