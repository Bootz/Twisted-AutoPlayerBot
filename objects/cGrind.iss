objectdef cGrind inherits cBase
{
	...
	method LoadBestLocationSet(bool ForceSwitch)
	{
		...
		if ${UIElement[chkAssistMode@Overview@Pages@Twisted].Checked}
		{
			return
		}
	...
	{	
		if ${Msg.Find[You have slain]}
		{
			...
			if ${UIElement[chkKillSoundOn@Config@Pages@Twisted].Checked}
			{
				This:PlaySound["Blip"]
			}
			return
	...
	{
		variable int minutes = ${UIElement[cmbLocTimer@Grind@Pages@Twisted].SelectedItem}  /*will change this to UI element later */
...
objectdef oLocationSet
{
	...
	method Refresh()
	{
		...
		variable iterator indexx
		if ${This.Name.Equal[${UIElement[tlbLocations@Grind@Pages@Twisted].SelectedItem.Text}]}
		{	
