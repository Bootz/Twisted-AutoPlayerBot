objectdef cAutoequip inherits cBase
{
	...
	member EquipBag()
	{
		if ${POI.Type.NotEqual[MOB]} && !${Me.Ghost} && !${Me.Dead} && !${Bot.PauseFlag} && ${UIElement[chkAutoEQ@EQ@Pages@Twisted].Checked}
		{
	...
	{
		if ${UIElement[chkAutoEQ@EQ@Pages@Twisted].Checked}
		{
	...
	{
		if ${UIElement[Twisted].FindChild[Pages].Tab["EQ"].FindChild[sld${stat}](exists)}    
		{     		
	...
	{
		UIElement[Twisted].FindChild[Pages].Tab["EQ"].FindChild[sld${stat}]:SetValue[${This.GetSliderStat[${stat}]}]     
	}

	/* returns slider value as a float read by Modifier */
	member GetActualStat(string stat)
	{
		if ${UIElement[Twisted].FindChild[Pages].Tab["EQ"].FindChild[sld${stat}].Value} != 0    
		{ 
			return ${This.ActualValue[${UIElement[Twisted].FindChild[Pages].Tab["EQ"].FindChild[sld${stat}].Value}]}  
		}		
		return 0
	}
