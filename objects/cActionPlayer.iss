...
objectdef cActionPlayer inherits cBase
{
	...
	{
		if ${UIElement[chkHumanSoundOn@Config@HumanPages@Human@Pages@Twisted].Checked}
		{
			This:PlaySound[${wav}]			
		}
	}
	...
	method StatsLog()
	{
		...
		redirect -append "${Script.CurrentDirectory}/config/logs/StatsLog.txt" echo "  Twisted session for ${Me.Name} the ${Me.Race} ${Me.Class} "
	...
	{
		if ${UIElement[chkCaptureWhispers@Config@HumanPages@Human@Pages@Twisted].Checked} && ${Sender.NotEqual["${Me.Name}"]}
		{
			...
			UIElement[Status@Whispers@HumanPages@Human@Pages@Twisted]:Echo["${Text}"]
			This:Debug[${Text}]
	...
	method Initialize()
	{
		...
		if ${UIElement[chkTranslate@Config@HumanPages@Human@Pages@Twisted].Checked}
		{
