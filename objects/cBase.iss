objectdef cBase
{
	variable string Twisted_Status = "Initializing"	
	...
	method Output(string Text)
	{
		...
		UIElement[Twisted].FindChild[Console]:Echo["${Time.Time24}: ${Text}"]
		This:Debug[${Text}]
		if ${UIElement[chkLogOutput@Config@Pages@Twisted].Checked}
		{
	...
	{
		if ${UIElement[chkDebug@Config@Pages@Twisted].Checked}
		{
	...
	{
		if !${STATUS.Equal[${This.Twisted_Status}]}
		{
			This.Twisted_Status:Set[${STATUS}]
		}
	}
;Method for debug when GUI console is created	
	...
	method PlaySound(string soundwav)
	{
		...
		if ${UIElement[chkSoundOn@Config@Pages@Twisted].Checked} && ${LavishScript.RunningTime} > ${Bot.Sound_TimeDelay}
		{	
			switch ${soundwav}
			{		
				case Alert
